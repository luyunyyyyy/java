...
public class GourmetCoffeeGUI extends JPanel {
	...
	public  GourmetCoffeeGUI(Catalog initialCatalog) {
		...
		quantityTextField = new JTextField("", QUANTITY__TEXTFIELD_SIZE);//quantity ×ÜÊýÁ¿
	...
	private JPanel getDataFieldsPanel(ArrayList<DataField> dataFields) {
		...
		JPanel container = new JPanel(new GridLayout(dataFields.size(),1));
		JPanel tempContainer = null;
		for(DataField temp:dataFields){
			tempContainer = new JPanel();
			tempContainer.add(new JLabel(temp.getName()+": "));
			tempContainer.add(new JLabel(temp.getValue()));
	...
	class AddModifyListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			try{
				int quantityOfProduct = Integer.parseInt(quantityTextField.getText());
				String code = (String) catalogList.getSelectedValue();
				Product product = catalog.getProduct(code);
				OrderItem orderItem = new OrderItem(product,quantityOfProduct);
				for(OrderItem temp:currentOrder){
					if(temp.getProduct().equals(product)){
						temp.setQuantity(temp.getQuantity()+quantityOfProduct);
						return;
					}
				}
				currentOrder.addItem(orderItem);
				statusTextArea.setText("The OrderItem has been added.");
			}finally{
				DecimalFormat df = new DecimalFormat(".##");

				orderList.setListData(currentOrder.getItems());
				
				totalTextField.setText(df.format(currentOrder.getTotalCost()));
			}
	...
	class RemoveListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			OrderItem currentOrderItem =  orderList.getSelectedValue();
//			stdErr.println(currentOrderItem+"get code from ");
//			for(OrderItem temp:currentOrder){
//				if(temp.getProduct().getCode().equals(currentOrderItem));
//					currentOrder.removeItem(temp);
//					currentOrder.
//			}
			for(Iterator<OrderItem> it = currentOrder.iterator();it.hasNext();)
			{
				if(it.next().equals(currentOrderItem)){
					it.remove();
				}
			}
			orderList.setListData(currentOrder.getItems());
			statusTextArea.setText("The orderItem has been removed.");
			totalTextField.setText(Double.toString(currentOrder.getTotalCost()));
	...
	class SaveSalesListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			try {
				PrintWriter out = new PrintWriter(new FileWriter("sales.txt"),true);
				out.println(sales);
				stdErr.print(sales);
			} catch (IOException e) {
